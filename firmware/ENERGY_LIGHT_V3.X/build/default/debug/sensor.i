...
void InitINA220(sensor_config_t device)
{
    ...
    writeINA220(0x00, device.config , device.addressI2C);
    writeINA220(0x05,device.calib, device.addressI2C);
...
void getINA220Data(sensor_t *device)
{
    ...
    writeINA220(0x00, device->sensor_config.config , device->sensor_config.addressI2C);
    writeINA220(0x05,device->sensor_config.calib, device->sensor_config.addressI2C);

    buff = readINA220(0x02, device->sensor_config.addressI2C);
    buff = buff >> 3;
    buff &= 0x1FFF;

    device->sensor_data.voltage.pWrite++;

    if(device->sensor_data.voltage.pWrite == 8)
    {
        device->sensor_data.voltage.pWrite = 0;
        device->sensor_data.voltage.max = 0x8000;
        device->sensor_data.voltage.min = 0x7FFF;
    }

    device->sensor_data.voltage.hist[device->sensor_data.voltage.pWrite] = buff;

    buff = readINA220(0x01, device->sensor_config.addressI2C);

    device->sensor_data.current.pWrite++;

    if(device->sensor_data.current.pWrite == 8)
    {
        device->sensor_data.current.pWrite = 0;
        device->sensor_data.current.max = 0x8000;
