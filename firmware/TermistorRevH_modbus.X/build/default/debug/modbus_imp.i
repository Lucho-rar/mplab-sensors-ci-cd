...
nmbs_error handle_read_coils(uint16_t address, uint16_t quantity, nmbs_bitfield coils_out, uint8_t unit_id, void *arg) {
  if (address + quantity > 2)
    return NMBS_EXCEPTION_ILLEGAL_DATA_ADDRESS;

  nmbs_bitfield *server_coils = &((mod_bus_registers*) arg)->server_coils.coils;

  for (int i = 0; i < quantity; i++) {
    _Bool value = ((_Bool) ((*server_coils)[(address + i) / 8] & (0x1 << ((address + i) % 8))));
    (((coils_out)[(i) / 8]) = ((value) ? (((coils_out)[(i) / 8]) | (0x1 << ((i) % 8))) : (((coils_out)[(i) / 8]) & ~(0x1 << ((i) % 8)))));
  }
...
nmbs_error handle_write_single_coil(uint16_t address, _Bool coils, uint8_t unit_id, void *arg) {
  ...
  nmbs_bitfield *server_coils = &((mod_bus_registers*) arg)->server_coils.coils;
  (((*server_coils)[(address) / 8]) = ((coils) ? (((*server_coils)[(address) / 8]) | (0x1 << ((address) % 8))) : (((*server_coils)[(address) / 8]) & ~(0x1 << ((address) % 8)))));

  return NMBS_ERROR_NONE;
}

nmbs_error handler_read_input_registers(uint16_t address, uint16_t quantity, uint16_t* registers_out, uint8_t unit_id, void *arg) {
  if (address + quantity > 3)
    return NMBS_EXCEPTION_ILLEGAL_DATA_ADDRESS;
...
void default_values_register(mod_bus_registers* registers)
{
    registers->server_holding_register.addr_slave = 100;
    registers->server_holding_register.baudrate = 9600;
    ...
    registers->server_input_register.serial_number = 666;
