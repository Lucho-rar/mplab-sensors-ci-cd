...
void main(void)
{
    ...
    getDeviceAddress(&sensor.sensor_info);
    ...
    platform_conf.arg = &(pack.registers);
    ...
    nmbs_t nmbs;
    nmbs_error err = nmbs_server_create(&nmbs, pack.registers.server_holding_register.addr_slave, &platform_conf, &callbacks);
    ...
    while(1)
    {
        ...
        if(((_Bool) ((pack.registers.server_coils.coils)[(0) / 8] & (0x1 << ((0) % 8)))))
        {
            pack.registers.server_input_register.sensor_value = sensor.sense(&sensor.sensor_info.ADCChannel);
            memset(pack.registers.server_coils.coils, 0, sizeof(nmbs_bitfield));
        }
    }
    exit(0);
