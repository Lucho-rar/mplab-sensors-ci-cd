...
nmbs_error handle_write_multiple_coils(uint16_t address, uint16_t quantity, const nmbs_bitfield coils, uint8_t unit_id, void *arg) {
  ...
  nmbs_bitfield *server_coils = &((mod_bus_registers*) arg)->server_coils.coils;
  for (int i = 0; i < quantity; i++) {
    (((*server_coils)[(address + i) / 8]) = ((((_Bool) ((coils)[(i) / 8] & (0x1 << ((i) % 8))))) ? (((*server_coils)[(address + i) / 8]) | (0x1 << ((address + i) % 8))) : (((*server_coils)[(address + i) / 8]) & ~(0x1 << ((address + i) % 8)))));
  }
...
void default_values_register(mod_bus_registers* registers)
{
    registers->server_holding_register.addr_slave = 1;
    registers->server_holding_register.baudrate = 9600;
    registers->server_holding_register.parity = 0;
    registers->server_holding_register.sensor_id = 1;
}
